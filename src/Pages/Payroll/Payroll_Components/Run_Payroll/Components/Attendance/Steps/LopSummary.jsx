...
const LopSummary = () => {
    ...
    setRows((prev) =>
      prev.map((row) => {
        if (row.id === id) {
          ...
          // Compute final lop manually
          const actual = parseDays(updatedRow.actual_lop);
          const adjust = parseInt(updatedRow.lopAdjustment) || 0;
          updatedRow.finalLop = `${Math.max(actual - adjust, 0)} day${
            Math.max(actual - adjust, 0) === 1 ? "" : "s"
          }`;
  ...
  const columns = [
      ...
      renderCell: (params) => (
        <div className="h-full flex justify-center items-center">
          <TextField
            ...
            variant="outlined"
            type="number"
            value={params.row.lopAdjustment}
            onChange={(e) =>
              handleChange(params.row.id, "lopAdjustment", e.target.value)
            }
            inputProps={{ min: 0 }}
          />
        </div>
    ...
    {
      field: "finalLop",
      headerName: "Final Lop",
      flex: 1,
      renderCell: (params) => params.row.finalLop || "-",
    },
    {
      field: "comment",
      headerName: "Comment",
      ...
      renderCell: (params) => (
        <div className="h-full flex justify-center items-center">
          <TextField
            ...
            onChange={(e) =>
              handleChange(params.row.id, "comment", e.target.value)
            }
          />
        </div>
      ),
    ...
    <div className="flex flex-col gap-10">
      ...
      <div className="w-full flex flex-col gap-6">
        ...
        <div className="w-full bg-[#EBF1FF] border-2 border-[#005377] text-[#19396F] font-normal text-[16px] rounded-xl py-2 px-3 font-poppins">
          LOP summaries for all employees this month will be displayed below.
          Admin can override LOP days if needed.
        </div>
      </div>
